...
public class Character : MonoBehaviour
{
    ...
    void Init()
    {
        ...
        {
            if (statsStruture.values[i] is ValueFloat)
            {
                statsContainer.valueList.Add(new ValueFloatReference(statsStruture.values[i], 0f));
            }
            if (statsStruture.values[i] is ValueInt)
            {
                statsContainer.valueList.Add(new ValueIntReference(statsStruture.values[i], 0));
            }
        }
    }
